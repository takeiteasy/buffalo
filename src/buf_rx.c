/*
 * buf_rx.c -- definitions for buf_rx.h (the .bflo spec + regex reader).
 *
 * Pure C, no cccc dependency. Compiled twice: by the system `cc` for the
 * host unit tests, and inside cccc's comptime VM -- src/buf_comptime.c
 * pulls the declarations in with `#include @comptime "buf_rx.h"` and lists
 * this file on the cccc command line, which forwards these bodies into the
 * comptime program on demand. Never calls MacroErrorAt (see buf_rx.h).
 */
#include "buf_rx.h"
...
/* internal: complement a byte set in place */
